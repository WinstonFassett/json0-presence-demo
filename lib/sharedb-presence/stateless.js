...
const ShareDBError = Backend.Error
const PresenceTransformation = require('./presence-transformation')
...
DocPresence.prototype.submitPresence = function (data, callback) {
  if (data != null) {
    ...
    if (!PresenceTransformation.supportsType(this.doc.type)) {
      var doc = this.doc;
      return process.nextTick(function() {
    ...
    data = PresenceTransformation.createPresence(this.doc.type, data);
  }
  ...
  process.nextTick(this.flushPresence.bind(this));
};
...
DocPresence.prototype._processReceivedPresence = function (src, emit) {
  ...
  if (!this.doc.type || !PresenceTransformation.supportsType(this.doc.type)) {
  ...
  var data = PresenceTransformation.createPresence(this.doc.type, presence.p);

  // Transform against past ops
  for (var i = startIndex; i < this.cachedOps.length; i++) {
    var op = this.cachedOps[i];
    data = PresenceTransformation.transform(this.doc.type, data, op.op, presence.src === op.src);
  }

  // Transform against pending ops
  if (this.doc.inflightOp) {
    data = PresenceTransformation.transform(this.doc.type, data, this.doc.inflightOp.op, false);
  }

  for (var i = 0; i < this.doc.pendingOps.length; i++) {
    data = PresenceTransformation.transform(this.doc.type, data, this.doc.pendingOps[i].op, false);
  }
...
DocPresence.prototype._transformPresence = function (src, op) {
  ...
  if (op.op != null) {
    ...
    presenceData = PresenceTransformation.transform(this.doc.type, presenceData, op.op, isOwnOperation);
  } else {
    presenceData = null;
  }
...
DocPresence.prototype.flushPresence = function () {
  if(!this.inflight && this.pending) {
    ...
    this.pending = null;
    this.doc.connection._connectionPresence.sendPresence(this.doc, this.doc.presence[''], this.requestReply);
